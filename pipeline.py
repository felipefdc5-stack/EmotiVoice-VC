...
from modules.backends.seed_vc_backend import SeedVCBackend
...
def _auto_device(preferred: Optional[str] = None) -> torch.device:
    ...
    if hasattr(torch.backends, "mps") and torch.backends.mps.is_available():  # pragma: no cover - mac specific
        return torch.device("mps")
    return torch.device("cpu")


class IndexRVC:
    """High-level orchestrator combining Seed-VC conversion with IndexTTS emotions."""
    ...
    def __init__(
        ...
        *,
        ...
        LOGGER.info("Initialising IndexRVC on %s", self.device)

        self.backend = SeedVCBackend(device=str(self.device))

        ckpt_dir = Path(checkpoints_dir) if checkpoints_dir else Path(__file__).resolve().parent / "checkpoints"
        emo_cfg = EmotionBridgeConfig(
